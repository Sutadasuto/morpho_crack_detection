...
def calculate_median_and_mad(img, b=1.4826):
    ...
    freq_hist = np.concatenate((freq_hist[1:][::-1], freq_hist))  # Mirror histogram over y axis to get a normal-like distribution
    unrolled_hist = []
    ...
    abs_median_deviation = sorted(abs(np.array(unrolled_hist, dtype=np.float32) - m_j))
    if r == 0:
...
def get_accumulated_probability(probability_histogram):
    accumulated_probability = np.zeros(probability_histogram.shape, dtype=np.float32)
    prev_prob = 0
...
def get_frequency_histogram(img):
    intensity_frequencies = np.array([0 for i in range(256)], dtype=np.uint16)
...
def get_probability_histogram(img):
    ...
    intensity_probabilities = intensity_frequencies.astype(np.float32) / n_pixels
